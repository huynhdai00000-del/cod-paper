...
#: The frozen matrix contains 7 seeds per cell. Below this many *converged*
#: seeds on a side, the min-max range is not a range and bar (b) is not
#: evaluable.
...
def build_report(runs: list, results_dir: Path) -> tuple:
    ...
    w("## 1. Cell coverage")
    w("")
    w(f"The frozen matrix contains {SEED_TARGET} seeds per declared cell. "
      "Convergence rate is reported before error statistics, and medians and "
      "full seed ranges use converged runs only.")
    w("")
    ...
    if cod is None or bnd is None:
        ...
        for m in METRICS:
            ...
            w(f"| {m.key} ({m.unit}) | {_cellstat(cod, m)} | "
              ...
              f"{_bar(cp.bar_a)} | {_bar(cp.bar_b)} | {cp.verdict} |")
        w("")
        w("Under the frozen protocol, the bounded formulation has a material "
          "convergence and accuracy cost and does not support a refinement "
          "claim.")
        w("")
